 ...
 */
async function searchFromApiWithPage(apiSite, query, page = 1, timeoutMs = 8000) {
  ...
  try {
    const apiUrl =
      page === 1
        ...
        : apiSite.url + API_CONFIG.search.pagePath.replace("{query}", encodeURIComponent(query)).replace("{page}", page.toString());
    ...
    const allResults = data.list.map((item) => {
      ...
      if (item.vod_play_url) {
        ...
        vodPlayUrlArray.forEach((url) => {
          ...
          titleUrlArray.forEach((titleUrl) => {
            ...
            if (episodeTitleUrl.length === 2 && episodeTitleUrl[1].endsWith(".m3u8")) {
      ...
      return {
        ...
        year: item.vod_year ? item.vod_year.match(/\d{4}/)?.[0] || "unknown" : "unknown",
...
export async function searchVideos(query, videoSources) {
  ...
  // 并行搜索所有源（只搜索第一页）
  const searchPromises = enabledSources.map((source) => searchFromApiWithPage(source, query, 1));
...
export async function getVideoDetail(id, sourceName, sourceUrl) {
  ...
  try {
    ...
    return {
      ...
      year: videoDetail.vod_year ? videoDetail.vod_year.match(/\d{4}/)?.[0] || "unknown" : "unknown",
